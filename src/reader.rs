    ...
    fn paint_layout(&mut self, reason: &wstr, is_final_rendering: bool) {
                ...
                shlex::split(&Into::<&[char]>::into(item.str()).iter().collect::<String>())
        {
            match &args[0] as &str {
                ...
                "git" => {
                    if args.len() >= 3 {
                        match &args[1] as &str {
                            "clone" => {
                                ...
                                if let Some(y) = iter.last() {
                                    if y.starts_with("https://") {
                                        ...
                                        {
                                            full_line =
                                                (WString::from(utf32str!("cd ")) + z).into();
                                            self.autosuggestion.text = full_line.clone().into();
                                        }
                            ...
                            "add" => {
                                full_line = WString::from(utf32str!("git commit")).into();
                                self.autosuggestion.text = full_line.clone().into();
                            }
                            _ => {}
                        }
                ...
                "gcc" => {
                    full_line = WString::from(utf32str!("./a.out")).into();
                    self.autosuggestion.text = full_line.clone().into();
                }
                "rustc" => {
                            ...
                            match extension {
                                "rs" => {
                                    if file_name == "main.rs" || file_name == "lib.rs" {
                                        full_line = WString::from(format!("cargo build",)).into();
                                        self.autosuggestion.text = full_line.clone().into();
                                    } else if file_name.contains("examples") {
                                        ...
                                        full_line =
                                            WString::from(format!("rustc {file_name}")).into();
                                        self.autosuggestion.text = full_line.clone().into();
                                    }
                                }
                                "lua" => {
                                    full_line = WString::from(format!("lua {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "cpp" => {
                                    full_line = WString::from(format!("g++ {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "c" => {
                                    full_line = WString::from(format!("gcc {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "dot" => {
                                    full_line = WString::from(format!("tod {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "lean" => {
                                    full_line = WString::from(format!("lean {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "mlw" => {
                                    full_line =
                                        WString::from(format!("why3 ide -L . {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "py" => {
                                    full_line =
                                        WString::from(format!("python3 {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "hs" => {
                                    full_line = WString::from(format!("ghc {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                "kk" => {
                                ...
                                "yaml" => {
                                    full_line =
                                        WString::from(format!("judicious {file_name}")).into();
                                    self.autosuggestion.text = full_line.clone().into();
                                }
                                _ => {}
                            }
                ...
                "review" => {
                    full_line = WString::from("sqlitebrowser ~/.goldendict/history.db").into();
                    self.autosuggestion.text = full_line.clone().into();
                }
                "sqlitebrowser" => {
                    if let Some(target) = args.get(1)
